<script context="module">
    import { isPostStore, currentPostPage, currentPost, currentPostPageId, postAuthor } from "$lib/stores/postStore";
        ...
        if (postSlug !== "1") {
            ...
            if (posts) {
                ...
                postOBJ.pages = projects.data.map( (x, i) => {
                    ...
                    if (typeof(rBody) != "undefined" && rBody.hasOwnProperty('files') && typeof(rBody.files) != "undefined") {
...
<script>
    ...
    // Stores
    import { htmlStore, cssStore, jsStore, triggerReset, toggleUpdateCodeWithPost, initialPostData, showCaptureThumbnail } from '$lib/stores/codeStore.js';
...
<div class="post-contentContainer" style="--contentWidth: calc({contentWidth}px - 2rem);">
    <div class="post-content" bind:clientWidth={contentWidth}>
        {#if editing} 
                ...
                <div class="post-details" in:slide="{{delay: 0, duration: 100}}" out:slide="{{delay: 100, duration: 150}}">
                    <div class="row">
                        <label for="post-title">
                            ...
                            <input type="text" bind:value={currentPostTitle} name="post-title" placeholder="Add post title..." />
                        </label>
            ...
            {#if showCodeDetails}
                <label for="" in:slide="{{delay: 0, duration: 100}}" out:slide="{{delay: 100, duration: 150}}">
            ...
            <div class="grow-wrap" bind:this={textAreaGrowContainer}>
                <textarea name="postText" placeholder="Page body..." bind:value={page.description} on:input={() => {
        ...
        <NewComment postId={$currentPost.id} parentId={null} tag={["Comment"]} on:commentPosted={() => {
            fetchComments();
        }} />
            
            <div class="post-control-bar">
                <Vote itemID={$currentPost.id} isPost={true} />
                <button class="comment-button cancel" on:click={() => (showCreateComment = false)}>Cancel</button>
            </div>
        {:else}
            <div class="post-control-bar">
                <Vote itemID={$currentPost.id} isPost={true} />
